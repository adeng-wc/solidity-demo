...
contract BlindAuction {
    ...
    /// 大意就是address类型被分成了address和address payable
    /// 我直接声明的addr1显然只是address
    ...
    //披露你的秘密竞拍出价。对于所有正确披露的无效出价以及除最高出价以外的所有出价，你都将获得退款
    function reveal(
        ...
        require(_values.length == length, " _values length not equeal.");
        require(_fake.length == length, " _fake length not equeal.");
        require(_secret.length == length, " _secret length not equeal.");

        uint256 refund;
        for (uint256 i = 0; i < length; i++) {
            Bid storage bidObj = bids[msg.sender][i];
            ...
            refund += bidObj.deposit;
            if (!fake && bidObj.deposit >= value) {
                if (placeBid(msg.sender, value)) refund -= value;
            }
            // 使发送者不可能再次认领同一笔订金
            bidObj.blindedBid = bytes32(0);
        }
